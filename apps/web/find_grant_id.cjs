    ...
    if (stat.isDirectory()) {
      ...
      if (fs.existsSync(transcriptPath)) {
        ...
        for await (const line of rl) {
          lineNum++;
          if (line.includes('63917b1a')) {
            try {
              const obj = JSON.parse(line);
              const contentLen = (obj.content || '').length;
              console.log(`Conv: ${file} | Line ${lineNum} | step_index=${obj.step_index} | type=${obj.type} | source=${obj.source} | contentLen=${contentLen}`);
            } catch (err) {
